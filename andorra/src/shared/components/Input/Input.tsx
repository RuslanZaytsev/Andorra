...
export const Input = ({
  ...
  const handleCLear = (): void => {
    ...
    if (isControlled) {
      onChange?.(emptyValue);
    } else setInternalValue(defaultValue);
  };
  ...
  const handleKeyDownNumber = (event: React.KeyboardEvent<HTMLInputElement>): void => {
    const allowedKeys = [
      'Backspace',
      'ArrowLeft',
      'ArrowRight',
      ...
      return (
        <div className={styles.wrapper}>
          <div className={styles.input_wrapper}>
            {showPassword ? (
              <EyeClose className={styles.icon} onClick={togglePasswordVisibility} />
            ) : (
              <EyeOpen className={styles.icon} onClick={togglePasswordVisibility} />
            )}
            <RootInput
              type={inputType}
              choiseValue={choiseValue}
